      ...
      <div className="grid min-w-0 gap-9 lg:grid-cols-2 lg:gap-14">
        <motion.div initial={{ opacity: 0, x: -30 }} animate={{ opacity: 1, x: 0 }} className="min-w-0 lg:sticky lg:top-28 lg:self-start">
          <div className="group relative overflow-hidden border border-white/10 bg-stone-900">
            ...
            />
            <span className="absolute left-4 top-4 border border-amber-400/30 bg-black/60 px-3 py-2 text-[9px] uppercase tracking-[0.22em] text-amber-400 backdrop-blur">
              LUXE Certified
            </span>
          </div>
        ...
        <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: .7 }} className="min-w-0">
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3">
            ...
            <span className="text-xs text-stone-500">{watch.rating} · Collector rated</span>
          </div>
          ...
          <div className="mt-8 grid grid-cols-2 gap-px overflow-hidden border border-white/10 bg-white/10 sm:grid-cols-3">
            {[
              ...
              ["Strap Material", "Premium Leather"], 
              ["Movement", watch.movement], 
              ["Water Resistance", watch.waterResistance], 
              ["Dial Color", "Obsidian Black"], 
              ["Warranty", "2 Years"]
            ].map(([label, value]) => (
              <div key={label} className="min-w-0 bg-stone-950 p-4 sm:p-5">
                <p className="text-[9px] uppercase tracking-[0.16em] text-stone-500">{label}</p>
                <p className="mt-2 break-words text-xs leading-5 text-white sm:text-sm">{value}</p>
              </div>
            ))}
          </div>
...
className="btn-outline w-full flex items-center justify-center ![border-color:rgba(255,255,255,0.2)] !text-white hover:![border-color:#fbbf24] hover:!bg-amber-400 hover:!text-black"            >
          ...
          <div className="mt-7 grid grid-cols-3 gap-2 border-y border-white/10 py-5 sm:gap-3">
            {[
              [ShieldCheck, "Authentic"], 
              [Truck, "Insured"], 
              [Zap, "Fast Dispatch"]
            ].map(([Icon, text]) => (
              <div key={text} className="text-center">
                <Icon size={18} className="mx-auto text-amber-400" />
                <p className="mt-2 text-[8px] uppercase tracking-[0.12em] text-stone-500 sm:text-[9px]">{text}</p>
              </div>
            ))}
          </div>
        </motion.div>
      ...
      {related.length > 0 && (
        <section className="mt-20 sm:mt-28">
          <div className="mb-8 sm:mb-10">
            ...
            <h2 className="heading-md mt-5 text-white">Related Timepieces</h2>
          </div>
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {related.map((item) => (
              <Link to={`/watch/${item.id}`} key={item.id} className="group border border-white/10 bg-stone-900 p-3.5 sm:p-4">
                <div className="overflow-hidden">
                  <img src={item.image} alt={item.name} className="aspect-square w-full object-cover transition duration-700 group-hover:scale-105" />
                </div>
