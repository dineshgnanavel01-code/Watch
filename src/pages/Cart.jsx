...
const Cart = () => {
  ...
  const [paymentMethod, setPaymentMethod] = useState(
    paymentMethods.some((method) => method.id === initialPayment) ? initialPayment : "card"
  );
  ...
  if (cart.length === 0) {
    return (
      <main className="flex min-h-[72vh] w-full items-center justify-center px-4 py-16 sm:px-6">
        <motion.div initial={{ opacity: 0, y: 25, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ duration: 0.7, ease: [0.22, 1, 0.36, 1] }} className="w-full max-w-xl border border-white/10 bg-stone-900/80 p-8 text-center shadow-2xl sm:p-14">
          <motion.div animate={{ y: [0, -7, 0], rotate: [0, -3, 3, 0] }} transition={{ duration: 3.5, repeat: Infinity, ease: "easeInOut" }} className="mx-auto flex h-16 w-16 items-center justify-center rounded-full border border-amber-400/30 bg-amber-400/10 text-amber-400">
          ...
          <p className="mx-auto mt-4 max-w-md text-sm leading-7 text-stone-400">Discover a timepiece worthy of your collection, crafted for moments that last.</p>
  ...
  return (
    <main className="mx-auto w-full max-w-7xl overflow-hidden px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
      <motion.div initial={{ opacity: 0, y: 25 }} animate={{ opacity: 1, y: 0 }} className="mb-10 sm:mb-14">
      ...
      <div className="grid min-w-0 gap-8 lg:grid-cols-[minmax(0,1fr)_390px] lg:gap-12">
        <div className="min-w-0 space-y-4">
          {cart.map((item, index) => (
            <motion.article key={item.id} initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.06, duration: 0.5 }} whileHover={{ y: -3 }} className="flex min-w-0 gap-3 border border-white/10 bg-stone-900 p-3 transition duration-300 hover:border-amber-400/30 hover:shadow-[0_15px_45px_rgba(0,0,0,.2)] sm:gap-5 sm:p-4">
        ...
        <motion.aside initial={{ opacity: 0, x: 25 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15, duration: 0.6 }} className="h-fit border border-white/10 bg-stone-900 p-5 shadow-xl sm:p-7 lg:sticky lg:top-28">
          ...
          <div className="mt-8 border-t border-white/10 pt-7">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-xs font-semibold uppercase tracking-[0.16em]">Payment Details</h3>
            </div>
            <p className="mt-2 text-xs leading-5 text-stone-500">Choose your preferred payment method before continuing.</p>
            ...
            <div className="mt-5 space-y-3">
              {paymentMethods.map((method) => {
                ...
                return (
                  <motion.button key={method.id} type="button" whileHover={{ x: 2 }} whileTap={{ scale: 0.99 }} onClick={() => setPaymentMethod(method.id)} className={`flex w-full items-center gap-3 border p-3 text-left transition duration-300 ${selected ? "border-amber-400/60 bg-amber-400/10 shadow-[0_0_25px_rgba(245,158,11,.06)]" : "border-white/10 bg-black/10 hover:border-white/20"}`}>
                    <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full border ${selected ? "border-amber-400/50 text-amber-400" : "border-white/10 text-stone-500"}`}><Icon size={16} /></div>
                    <div className="min-w-0 flex-1">
                      <p className={`text-xs font-medium ${selected ? "text-amber-400" : "text-white"}`}>{method.name}</p>
                      <p className="mt-1 text-[10px] text-stone-500">{method.description}</p>
                    </div>
                    <div className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border ${selected ? "border-amber-400 bg-amber-400 text-black" : "border-white/20 text-transparent"}`}><Check size={12} strokeWidth={3} /></div>
                  </motion.button>
                );
              })}
            </div>
          ...
          <div className="mt-6 flex items-start gap-3 border border-emerald-400/10 bg-emerald-400/5 p-3">
            ...
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-emerald-400">Secure Checkout</p>
              <p className="mt-1 text-[10px] leading-5 text-stone-500">Payment details are handled securely during checkout.</p>
            </div>
          </div>

          <Link to={`/checkout?payment=${paymentMethod}`} className="group relative mt-7 flex min-h-14 w-full items-center justify-center gap-3 overflow-hidden bg-amber-500 px-5 text-xs font-semibold uppercase tracking-[0.16em] text-black transition duration-300 hover:-translate-y-1 hover:bg-amber-400 hover:shadow-[0_16px_40px_rgba(245,158,11,.2)]">
          ...
          <div className="mt-4 flex items-center justify-center gap-2 text-[9px] uppercase tracking-[0.18em] text-stone-500"><ShieldCheck size={13} className="text-amber-400" /> Secure & insured purchase</div>
          <Link to="/collection" className="group mt-4 flex min-h-11 items-center justify-center gap-2 border border-white/10 text-xs uppercase tracking-[0.14em] text-stone-400 transition hover:border-amber-400/40 hover:text-white"><ArrowLeft size={14} className="transition-transform group-hover:-translate-x-1" /> Continue Shopping</Link>
          <p className="mt-4 text-center text-[9px] uppercase tracking-[0.2em] text-stone-600">Payment: {paymentMethods.find((method) => method.id === paymentMethod)?.name}</p>
        </motion.aside>
      </div>
    </main>
  );
